...
@section('header')
    ...
    <div class="flex items-center gap-4">
        <form action="{{ route('patients.index') }}" method="GET" class="relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by Name or IC..."
                class="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 w-64">
        </form>
        <a href="{{ route('export.csv') }}"
            class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition">
                    ...
                    @forelse($patients as $patient)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-gray-600">
                                {{ $patient->date_of_visit ? $patient->date_of_visit->format('M d, Y') : '' }}</td>
                            <td class="px-6 py-4 font-semibold {{ $patient->hba1c_color }}">{{ $patient->hba1c }}%</td>
                            <td class="px-6 py-4 text-gray-600">
                                {{ $patient->next_follow_up ? $patient->next_follow_up->format('M d, Y') : '-' }}</td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <a href="{{ route('patients.hl7', $patient) }}"
                                        class="p-2 text-purple-600 hover:bg-purple-50 rounded-lg transition" title="Export HL7">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p>No patients found.
                                    {{ request('search') ? 'Try a different search.' : 'Add your first patient to get started.' }}
                                </p>
                            </td>
                        </tr>
                    @endforelse
